...
class ObsFeat2D(nn.Module):
    ...
    def __init__(self, n_lat, n_points, n_out=1024):
        super(ObsFeat2D, self).__init__()
        self.n_out = n_out
        k = 3
        p = int(np.floor(k / 2)) + 2
        self.n_lat = n_lat
        self.conv1 = nn.Conv1d(2+n_lat, 64, kernel_size=k, padding=p, dilation=3)
    ...
    def forward(self, x):
        assert(x.shape[1] == 2+self.n_lat), "the input size must be <Bx2xL> "
...
class LocNetReg2D(nn.Module):
    def __init__(self, n_lat, n_points, out_dims):
        super(LocNetReg2D, self).__init__()
        self.obs_feat_extractor = ObsFeat2D(n_lat, n_points)
