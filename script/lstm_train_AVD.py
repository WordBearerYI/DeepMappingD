...
parser.add_argument('-d','--data_dir',type=str,default='None',help='dataset path')
...
parser.add_argument('--scene_index',type=int,default=None,help='dd')
...
scheduler = StepLR(optimizer, step_size=3000, gamma=0.6)
...
for epoch in range(opt.n_epochs):
    ...
    for index,(obs_batch,valid_pt,latent_indexes) in enumerate(loader):
        ...
        optimizer.zero_grad()
        loss.backward()
        #print('start',latent_vecs[lats_inds[0]])
        #print(latent_vecs[lats_inds[1]].grad)
        optimizer.step()
        #print('end',latent_vecs[lats_inds[0]])
    ...
    if (epoch+1) % (5*opt.log_interval) == 0:
        ...
        with torch.no_grad():
            ...
            os.system('./lstm_run_eval_vis_AVD.sh '+str(opt.scene_index))
