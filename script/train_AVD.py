...
scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'min',threshold=0.001,verbose=1,patience=6)
...
for epoch in range(opt.n_epochs):
    ...
    for index,(obs_batch,valid_pt,latent_indexes) in enumerate(loader):
        ...
        for i_lat in latent_indexes.cpu().detach().numpy():
            if opt.mode=='z_single':
                latent = latent_vecs[i_lat].unsqueeze(-1)
            elif opt.mode=='maxpool_single':
                latent = maxpool_1D(torch.stack(latent_vecs)).unsqueeze(-1)
            else:
                start = i_lat-pad_req_num
                end = i_lat+pad_req_num-1
                if start<0:
                    latent = [latent_vecs[0] if i_c<0 else latent_vecs[i_c] for i_c in range(start,end)]
                    #latent = torch.cat([latent_vecs[i_lat],latent_vecs[i_lat]],0)
                elif end>instances_per_scene-1:
                    latent = [latent_vecs[i_c] if i_c<=instances_per_scene-1 else latent_vecs[-1] for i_c in range(start,end)]
                else: 
                    latent = [latent_vecs[i_c] for i_c in range(start,end)]
                latent = maxpool_1D(torch.stack(latent)).unsqueeze(-1)
